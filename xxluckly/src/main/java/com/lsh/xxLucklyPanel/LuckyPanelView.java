...
public class LuckyPanelView extends FrameLayout {
    ...
    public LuckyPanelView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
    ...
    private void setupView() {
        ...
        for (ItemView itemView : itemViewArr) {
            mImageView.add(itemView.getImageView());
        }
    }

    public void setTextContent(List<String> datas) {
        if (datas.size() != 8) {
            throw new IllegalArgumentException("抽奖转盘的Content数据必须为8个");
        }
        for (int i = 0; i < 8; i++) {
            itemViewArr[i].setTextContent(datas.get(i));
        }
    }

    public void setImageContent(List<Bitmap> datas) {
        if (datas.size() != 8) {
            throw new IllegalArgumentException("抽奖转盘的Content数据必须为8个");
        }
        for (int i = 0; i < 8; i++) {
            itemViewArr[i].setImageContent(datas.get(i));
        }
    }

    public void setImageContent(ImageViewListener imageViewListener) {
        for (int i = 0; i < 8; i++) {
            itemViewArr[i].setTextGone();
        }
        imageViewListener.onImageView(mImageView);
    }
